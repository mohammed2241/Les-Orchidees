...
def creer_pdf_section(titre, data_list, type_rapport):
    ...
    if not data_list:
        ...
        pdf.cell(0, 10, "Aucune donnee enregistree dans cette section.", ln=True)
    ...
    if type_rapport == "marchandises":
        # Correction du tableau Marchandises
        ...
        pdf.cell(35, 10, 'LOT/REF', 1, 0, 'C', True)
        pdf.cell(65, 10, 'LIEU EXACT', 1, 0, 'C', True)
        pdf.cell(30, 10, 'SURFACE', 1, 1, 'C', True)
        
        pdf.set_font('Arial', '', 8)
        for r in data_list:
            pdf.cell(25, 8, str(r.get('Date', '-')), 1)
            pdf.cell(35, 8, str(r.get('Type', '-')), 1)
            pdf.cell(35, 8, str(r.get('Référence', '-')) if r.get('Référence') else '-', 1)
            pdf.cell(65, 8, str(r.get('Lieu', '-'))[:45], 1)
            pdf.cell(30, 8, f"{r.get('Surface', '-')} m2", 1, 1)
...
if mode == "📝 SAISIE":
    ...
    with tabs[2]: # Suivi métiers
        ...
        if spec == "Marbre":
            ...
            z = st.selectbox("Zone", ["SDB", "Chambre", "Terrasse"]); im = st.text_input("Immeuble"); et = st.selectbox("Etage", ["RDC","1","2","3","4"]); p = st.file_uploader("Photo")
            if st.button("Enregistrer Céramique"):
                data['ceram'].append({"Type":z, "Lieu":f"Imm {im} - {et}", "Date":pd.Timestamp.now().strftime("%d/%m"), "photo":p.getvalue() if p else None})
                sauvegarder_donnees(); st.success("Céramique OK")
        else:
            items = ["Spot", "Prise TV"] if spec == "Électricité" else ["Vasque", "Toilette"]
            for i in items:
                c1, c2 = st.columns([3,1]); q = c2.number_input("Qté", min_value=0, key=f"q_{i}"); dt = c1.text_input(f"Détails {i}", key=f"d_{i}")
                if st.button(f"Valider {i}", key=f"b_{i}"):
                    ...
                    sauvegarder_donnees(); st.toast("Enregistré")
    ...
    with ctabs[0]: # Plans
        for p in data['plans']:
            with st.expander(f"📄 {p['nom']}"):
                st.download_button("Ouvrir le Plan", data=p['content'], file_name=p['nom'])
                if p.get('type','').startswith('image'): st.image(p['content'])

    with ctabs[1]: # Marchandises (Rétablie et PDF corrigé)
        st.download_button("📥 Télécharger Rapport Marchandises (PDF)", 
                           ...
                           file_name=f"Marchandises_{tranche}.pdf")
    ...
    with ctabs[2]: # Suivi métiers
        ...
        st.download_button(f"📥 Rapport {m_sel} (PDF)", 
                           data=creer_pdf_section(m_sel.upper(), data[k_map[m_sel]], k_map[m_sel]), 
                           file_name=f"Rapport_{m_sel}.pdf")
        for entry in data[k_map[m_sel]]:
